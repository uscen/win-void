...
(function () {
  ...
  function declineCookies() {
    ...
    runningNotif.style.cssText =
      "position:fixed;top:10px;right:10px;background:orange;color:white;padding:5px;z-index:99999;font-size:12px;";
    ...
    for (const selector of declineSelectors) {
      ...
      for (const element of elements) {
        if (element && element.offsetParent !== null && !element.disabled) {
          console.log(
            "Cookie Auto Decline: Found attribute-based button",
            element,
          );
          element.click();
          foundButton = true;
    ...
    if (!foundButton) {
      const allButtons = document.querySelectorAll(
        'button, a[role="button"], input[type="button"], input[type="submit"], span[role="button"], div[role="button"]',
      );
      ...
      for (const button of allButtons) {
        ...
        if (
          ...
          debug.style.cssText =
            "position:fixed;top:10px;right:10px;background:red;color:white;padding:5px;z-index:99999;font-size:12px;";
        ...
        if (isRejectButton) {
          ...
          if (!isAcceptButton) {
            ...
            notification.style.cssText =
              "position:fixed;top:10px;left:10px;background:green;color:white;padding:10px;z-index:99999;font-size:14px;border-radius:5px;";
      ...
      bannerSelectors.forEach((selector) => {
        ...
        banners.forEach((banner) => {
          if (banner.offsetParent) {
            ...
            bannerNotif.style.cssText =
              "position:fixed;top:50px;left:10px;background:blue;color:white;padding:5px;z-index:99999;font-size:12px;border-radius:3px;";
